...
public class AdminUserPage extends AdminsBasePage {
    ...
    public AdminUserPage(PageParameters parameters) {
        ...
        String username;
        try {
            username = TripleDes.decrypt(professionalUserModel.getObject().getUsernameHash());
        } catch (Exception e) {
            username = "";
        }

        UserLabel usernameLabel = new UserLabel("usernameLabel", username);
        ...
        UserLabel passwordLabel = new UserLabel("passwordLabel", password);
        // hide this field if new user password will be autogenerated
        passwordLabel.setVisible(!newUser);
        passwordLabel.setHideable(false);
