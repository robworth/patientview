...
public class TherapyDaoImpl extends BaseDaoImpl implements TherapyDao {
    ...
    public void saveTherapy(final Therapy therapy) {
        ...
        if (therapy.hasValidId()) {
            ...
            getNamedParameterJdbcTemplate().update("UPDATE tbl_Therapy "
                    + "SET "
                    + "RADAR_NO = :RADAR_NO, "
    ...
    public Therapy getTherapy(long id) {
        try {
            return getJdbcTemplate().queryForObject("SELECT * FROM tbl_Therapy WHERE tID = ?", new Object[]{id},
    ...
    public List<Therapy> getTherapyByRadarNumber(long radarNumber) {
        return getJdbcTemplate().query("SELECT * FROM tbl_Therapy WHERE RADAR_NO = ?", new Object[]{radarNumber},
                new TherapyRowMapper());
    }

    public Therapy getFirstTherapyByRadarNumber(long radarNumber) {
        try {
            return getJdbcTemplate().queryForObject("SELECT * FROM tbl_Therapy WHERE RADAR_NO = ? AND SEQ_NO = 1",
