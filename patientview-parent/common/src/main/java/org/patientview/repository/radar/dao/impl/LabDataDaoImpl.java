...
import org.patientview.repository.radar.dao.LabDataDao;
...
public class LabDataDaoImpl extends BaseDaoImpl implements LabDataDao {
    ...
    public void saveLabData(final LabData labData) {
        ...
        if (labData.hasValidId()) {
            ...
            getNamedParameterJdbcTemplate().update("UPDATE tbl_LabData "
                    + "SET " + "RADAR_NO = :RADAR_NO, "
                    + "DATE_LAB_RES = :DATE_LAB_RES, " + "CREAT_SER = :CREAT_SER, "
                    + "PROTEIN = :PROTEIN, " + "ALBUMIN = :ALBUMIN, " + "UREA_SER = :UREA_SER, "
                    + "SODIUM = :SODIUM, " + "POTASSIUM = :POTASSIUM, "
                    + "PHOS = :PHOS, " + "PROT_CREAT_RAT = :PROT_CREAT_RAT, "
                    + "ALB_CREAT_RAT = :ALB_CREAT_RAT, " + "WBC = :WBC, "
                    + "HB = :HB, " + "NEUTRO = :NEUTRO, "
                    + "PLATELETS = :PLATELETS, " + "FERRITIN = :FERRITIN, "
                    + "CHOL_TOTAL = :CHOL_TOTAL, " + "CHOL_HDL = :CHOL_HDL, "
                    + "CHOL_LDL = :CHOL_LDL, " + "TRIG = :TRIG, "
                    + "CREAT_CLEAR_24_URINE = :CREAT_CLEAR_24_URINE, "
                    + "CREAT_CLEAR_RADIO = :CREAT_CLEAR_RADIO, "
                    + "CREAT_CLEAR_SCHZ = :CREAT_CLEAR_SCHZ, "
                    + "THYROX = :THYROX, " + "TSH = :TSH, "
                    + "ANCA = :ANCA," + "ELISA_ASS = :ELISA_ASS, "
                    + "ENA = :ENA, " + "ANA = :ANA, "
                    + "DNA_ANTIB = :DNA_ANTIB, " + "DNA_ANTI_DS = :DNA_ANTI_DS, "
                    + "CRYOGLOB = :CRYOGLOB, " + "ANTI_GBM = :ANTI_GBM, "
                    + "IGG = :IGG, " + "IGA = :IGA, "
                    + "IGM = :IGM, " + "COMP_C3 = :COMP_C3, "
                    + "COMP_C4 = :COMP_C4, " + "COMP_OTHER = :COMP_OTHER, "
    ...
    public LabData getLabData(long id) {
        try {
            return getJdbcTemplate().queryForObject("SELECT * FROM tbl_LabData WHERE labID = ?",
    ...
    public List<LabData> getLabDataByRadarNumber(long id) {
        return getJdbcTemplate().query("SELECT * FROM tbl_LabData WHERE RADAR_NO = ?",
                new Object[]{id}, new LabDataRowMapper());
    }

    public LabData getFirstLabDataByRadarNumber(Long id) {
        try {
            return getJdbcTemplate().queryForObject("SELECT * FROM tbl_LabData WHERE RADAR_NO = ? AND SEQ_NO = 1",
                    new Object[]{id}, new LabDataRowMapper());
        } catch (EmptyResultDataAccessException e) {
            return null;
            ...
            List<LabData> labDatas = getJdbcTemplate()
                    .query("SELECT * FROM tbl_LabData WHERE RADAR_NO = ? AND SEQ_NO = 1",
                            new Object[]{id}, new LabDataRowMapper());
            if (!labDatas.isEmpty()) {
