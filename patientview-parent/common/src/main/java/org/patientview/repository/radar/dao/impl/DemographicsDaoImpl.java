...
public class DemographicsDaoImpl extends BaseDaoImpl implements DemographicsDao {
    ...
    public void saveDemographics(final Patient patient) {
        ...
        if (patient.hasValidId()) { getJdbcTemplate().update(
                    "UPDATE patient SET "
                            + "rrNo = ?, "
                            + "dateReg = ?, "
                            + "nhsno = ?, "
            ...
            getJdbcTemplate().update("UPDATE patient set radarNo = ? WHERE id = ? ", id.longValue(), id.longValue());
    ...
    public Patient getDemographicsByRadarNumber(long radarNumber) {
        try {
            return getJdbcTemplate().queryForObject("SELECT * FROM patient WHERE radarNo = ?",
    ...
    public Patient getDemographicsByNhsNoAndUnitCode(String nhsNo, String unitCode) {
        try {
            return getJdbcTemplate().queryForObject("SELECT * FROM patient WHERE nhsno = ? AND unitcode = ? ",
    ...
    public List<Patient> getDemographicsByRenalUnit(Centre centre) {
        ...
        return getJdbcTemplate().query("SELECT pa.* "
                + "FROM "
                + "     ("
    ...
    public Sex getSex(long id) {
        try {
            return getJdbcTemplate().queryForObject("SELECT * FROM tbl_Sex WHERE sID = ?", new Object[]{id},
                    new SexRowMapper());
        } catch (EmptyResultDataAccessException e) {
    ...
    public Sex getSex(String sex) {
        try {
            return getJdbcTemplate().queryForObject("SELECT * FROM tbl_Sex WHERE sType = ?", new Object[]{sex},
                    new SexRowMapper());
        } catch (EmptyResultDataAccessException e) {
    ...
    public List<Sex> getSexes() {
        return getJdbcTemplate().query("SELECT * FROM tbl_Sex", new SexRowMapper());
    }
    ...
    public List<Status> getStatuses() {
        return getJdbcTemplate().query("SELECT * FROM tbl_Status", new StatusRowMapper());
    }
...
//
    ...
    public DemographicsUserDetail getDemographicsUserDetail(String nhsno, String unitcode) {
        ...
        params.add(nhsno);
        try {
            return getJdbcTemplate().queryForObject(sql, params.toArray(), new DemographicsUserDetailMapper());
        } catch (EmptyResultDataAccessException e) {
