...
import org.patientview.repository.radar.dao.TreatmentDao;
...
public class TreatmentDaoImpl extends BaseDaoImpl implements TreatmentDao {
    ...
    public void saveTreatment(final Treatment treatment) {
        Map<String, Object> treatmentMap = new HashMap<String, Object>() {
            {
                ...
                put("MODALITY", treatment.getTreatmentModality() != null
                        ? treatment.getTreatmentModality().getId() : null);
        ...
        if (treatment.hasValidId()) {
            ...
            getNamedParameterJdbcTemplate().update("UPDATE tbl_rrt_treatment "
    ...
    public void deleteTreatment(Treatment treatment) {
        Map<String, Object> treatmentMap = new HashMap<String, Object>();
        treatmentMap.put("tID", treatment.getId());
        getNamedParameterJdbcTemplate().update("DELETE FROM tbl_rrt_treatment "
                + "WHERE tID = :tID;", treatmentMap);
    }

    public Treatment getTreatment(long id) {
        try {
            return getJdbcTemplate().queryForObject("SELECT * FROM tbl_RRT_TREATMENT WHERE tID = ?", new Object[]{id},
    ...
    public List<Treatment> getTreatmentsByRadarNumber(long radarNumber) {
        return getJdbcTemplate().query("SELECT * FROM tbl_RRT_TREATMENT WHERE RADAR_NO = ?", new Object[]{radarNumber},
                new TreatmentRowMapper());
    }

    public TreatmentModality getTreatmentModality(long id) {
        try {
            return getJdbcTemplate().queryForObject("SELECT * FROM tbl_RRT_Modality WHERE mID = ?", new Object[]{id},
    ...
    public List<TreatmentModality> getTreatmentModalities() {
        return getJdbcTemplate().query("SELECT * FROM tbl_RRT_Modality WHERE mID <= 19;",
                new TreatmentModalityRowMapper());
    }
