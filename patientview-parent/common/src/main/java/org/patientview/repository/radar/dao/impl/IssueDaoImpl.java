...
import org.patientview.repository.radar.dao.IssueDao;
...
public class IssueDaoImpl extends BaseDaoImpl implements IssueDao {
    ...
    public Issue getIssue(Long id) {
        try {
            return getJdbcTemplate().queryForObject("SELECT * FROM tbl_IssueTracker WHERE bID = ?",
    ...
    public void saveIssue(final Issue issue) throws Exception {
        ...
        if (issue.hasValidId()) {
            ...
            getNamedParameterJdbcTemplate().update(updateSql, issueMap);
        } else {
    ...
    public void deleteIssue(Issue issue) throws Exception {
        ...
        getNamedParameterJdbcTemplate().update("DELETE FROM tbl_IssueTracker WHERE bID = :bID;", issueMap);
    }
    ...
    public List<Issue> getIssues(IssueFilter filter, int page, int numberPerPage) {
        ...
        sqlQueries.add("SELECT  * FROM  tbl_IssueTracker");
