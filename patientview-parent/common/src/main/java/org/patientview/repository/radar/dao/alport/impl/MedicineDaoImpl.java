package org.patientview.repository.radar.dao.alport.impl;
...
import org.patientview.repository.radar.dao.impl.BaseDaoImpl;
...
public class MedicineDaoImpl extends BaseDaoImpl implements MedicineDao {
    ...
    public void save(Medicine medicine) {
        ...
        if (medicine.hasValidId()) {
            getNamedParameterJdbcTemplate().update(buildUpdateQuery(TABLE_NAME, ID_FIELD_NAME, geneticsMap),
                    geneticsMap);
        } else {
    ...
    public void delete(Medicine medicine) {
        ...
        getNamedParameterJdbcTemplate().update("DELETE FROM " + TABLE_NAME + " WHERE " + ID_FIELD_NAME + " = :"
                + ID_FIELD_NAME, geneticsMap);
    }

    public Medicine get(Long id) {
        try {
            return getJdbcTemplate().queryForObject("SELECT * FROM " + TABLE_NAME + " WHERE " + ID_FIELD_NAME + " = ?",
    ...
    public List<Medicine> getMedicinesByNhsNo(String nhsNo) {
        try {
            return getJdbcTemplate().query("SELECT * FROM " + TABLE_NAME + " WHERE " + NHS_NO_FIELD_NAME + " = ?",
    ...
    public List<Medicine> getMedicinesByNhsNoAndDiseaseGroup(String nhsNo, DiseaseGroup diseaseGroup) {
        try {
            return getJdbcTemplate().query("SELECT * FROM " + TABLE_NAME + " WHERE " + NHS_NO_FIELD_NAME + " = ? AND "
