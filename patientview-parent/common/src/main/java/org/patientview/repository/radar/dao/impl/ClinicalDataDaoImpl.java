...
public class ClinicalDataDaoImpl extends BaseDaoImpl implements ClinicalDataDao {
    ...
    public void saveClinicalDate(final ClinicalData clinicalData) {
        ...
        if (clinicalData.hasValidId()) {
            ...
            getNamedParameterJdbcTemplate().update("UPDATE tbl_clinicalData "
    ...
    public ClinicalData getClinicalData(long id) {
        try {
            return getJdbcTemplate().queryForObject("SELECT * FROM tbl_ClinicalData WHERE cID = ?", new Object[]{id},
    ...
    public List<ClinicalData> getClinicalDataByRadarNumber(long radarNumber) {
        return getJdbcTemplate().query("SELECT * FROM tbl_ClinicalData WHERE RADAR_NO = ?",
                new Object[]{radarNumber}, new ClinicalDataRowMapper());
    }

    public ClinicalData getFirstClinicalDataByRadarNumber(long radarNumber) {
        try {
            return getJdbcTemplate().queryForObject("SELECT * FROM tbl_ClinicalData WHERE RADAR_NO = ? AND SEQ_NO = 1",
                    new Object[]{radarNumber}, new ClinicalDataRowMapper());
        } catch (EmptyResultDataAccessException e) {
            return null;
        }
    ...
    public Phenotype getPhenotype(long id) {
        try {
            return getJdbcTemplate().queryForObject("SELECT * FROM tbl_PHENOTYPES WHERE pID = ?", new Object[]{id},
    ...
    public List<Phenotype> getPhenotypes() {
        return getJdbcTemplate().query("SELECT * FROM tbl_PHENOTYPES", new PhenotypeDataRowMapper());
    }
