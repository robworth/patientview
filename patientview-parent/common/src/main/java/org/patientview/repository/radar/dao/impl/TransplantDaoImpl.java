...
import org.patientview.repository.radar.dao.TransplantDao;
...
public class TransplantDaoImpl extends BaseDaoImpl implements TransplantDao {
    ...
    public void saveTransplant(final Transplant transplant) {
        ...
        if (transplant.hasValidId()) {
            ...
            getNamedParameterJdbcTemplate().update("UPDATE tbl_transplant "
    ...
    public void deleteTransplant(Transplant transplant) {
        ...
        getNamedParameterJdbcTemplate().update("DELETE FROM tbl_transplant "
                + "WHERE trID = :trID;", transplantTreatmentMap);
    }

    public Transplant getTransplant(long id) {
        try {
            return getJdbcTemplate().queryForObject("SELECT * FROM tbl_Transplant WHERE trID = ?", new Object[]{id},
    ...
    public List<Transplant> getTransplantsByRadarNumber(long radarNumber) {
        return getJdbcTemplate().query("SELECT * FROM tbl_Transplant WHERE radar_no = ?", new Object[]{radarNumber},
                new TransplantRowMapper());
    }

    public List<Transplant.Modality> getTransplantModalitites() {
        return getJdbcTemplate().query("SELECT * FROM tbl_transplant_modality", new TransplantModalityRowMapper());
    }

    public Transplant.Modality getTransplantModality(long id) {
        try {
            return getJdbcTemplate().queryForObject("SELECT * FROM tbl_transplant_modality WHERE trID = ?",
                    new Object[]{id}, new TransplantModalityRowMapper());
        } catch (EmptyResultDataAccessException e) {
            return null;
        }
    ...
    public void saveRejectData(final Transplant.RejectData rejectData) {
        ...
        if (rejectData.hasValidId()) {
            ...
            getNamedParameterJdbcTemplate().update("UPDATE tbl_transplant_reject "
                    + "SET trID = :trID, "
    ...
    public void deleteRejectData(Transplant.RejectData rejectData) {
        Map<String, Object> rejectDataMap = new HashMap<String, Object>();
        rejectDataMap.put("recID", rejectData.getId());
        getNamedParameterJdbcTemplate().update("DELETE FROM tbl_transplant_reject "
                + "WHERE recID = :recID;", rejectDataMap);
    }

    public List<Transplant.RejectData> getRejectDataByTransplantNumber(Long transplantId) {
        return getJdbcTemplate().query("SELECT * FROM tbl_transplant_reject WHERE trID = ? AND trFailureDate IS NULL",
                new Object[]{transplantId}, new TransplantRejectRowMapper());
    }

    public Transplant.RejectData getRejectData(Long id) {
        try {
            return getJdbcTemplate().queryForObject("SELECT * FROM tbl_transplant_reject WHERE recID = ?",
                    new Object[]{id},
    ...
    private Transplant.RejectData getFailureDateRejectDataByTransplantNumber(Long transplantId) {
        try {
            return getJdbcTemplate().queryForObject("SELECT * FROM tbl_transplant_reject WHERE trID = ? AND "
