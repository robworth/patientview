...
import org.patientview.repository.radar.dao.PathologyDao;
...
public class PathologyDaoImpl extends BaseDaoImpl implements PathologyDao {
    ...
    public void savePathology(final Pathology pathology) {
        ...
        if (pathology.hasValidId()) {
            ...
            getNamedParameterJdbcTemplate().update("UPDATE tbl_pathology "
    ...
    public Pathology getPathology(long id) {
        try {
            return getJdbcTemplate().queryForObject("SELECT * FROM tbl_Pathology WHERE pID = ?", new Object[]{id},
    ...
    public List<Pathology> getPathologyByRadarNumber(long radarNumber) {
        return getJdbcTemplate().query("SELECT * FROM tbl_Pathology WHERE RADAR_NO = ?", new Object[]{radarNumber},
                new PathologyRowMapper());
    }
